...
class DataPreprocessor:
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Cleaned data: {len(df)} rows, {df.isnull().sum().sum()} NaN values remaining"
        )
    ...
    def add_macd(
        ...
        logger.info(
            f"Added MACD indicator (fast={fast_period}, slow={slow_period}, signal={signal_period})"
        )
    ...
    def add_stochastic(
        self, df: pd.DataFrame, period: int = 14, smooth_k: int = 3, smooth_d: int = 3
    ) -> pd.DataFrame:
        """
    ...
    def add_adx(self, df: pd.DataFrame, period: int = 14) -> pd.DataFrame:
        ...
        atr = (
            ...
            else self.add_atr(df, period)[f"atr_{period}"]
        )
    ...
    def add_all_indicators(
        ...
        # Try to add advanced indicators if available
        try:
            ...
            logger.info(
                "Added advanced indicators (volatility, order flow, microstructure)"
            )
        except ImportError as e:
            logger.debug(f"Advanced indicators not available: {e}")
    ...
    def split_data(
        ...
        logger.info(
            f"Split data: train={len(train)}, validation={len(validation)}, test={len(test)}"
        )

        return {"train": train, "validation": validation, "test": test}

    def create_sequences(self, data: np.ndarray, sequence_length: int = 60) -> tuple:
        """
    ...
    def prepare_for_ml(
        ...
        logger.info(
            f"Prepared data for ML: X_train={X_train.shape}, X_test={X_test.shape}"
        )
